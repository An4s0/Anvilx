...
export function initCommand({ name }) {
    if (!name) {
    ...
    if (fs.existsSync(targetDir)) {
        console.error(`anvilx: folder "${name}" already exists. Choose a different project name.`);
        return;
    }
    ...
    try {
        ...
        execSync("pnpm add -w -D typescript @types/node tailwindcss @tailwindcss/postcss postcss eslint prettier", { cwd: targetDir, stdio: "inherit" });
